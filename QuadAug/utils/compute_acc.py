...
def compute_performance_ctrain(end_task_acc_arr):
    t_coef = stats.t.ppf((1 + 0.95) / 2, end_task_acc_arr.shape[0] - 1)
    avg_acc = np.mean(end_task_acc_arr)
    CIs = t_coef * sem(end_task_acc_arr)
    return avg_acc,CIs


def compute_performance_train(end_task_acc_arr):
    """
